...
const divisibleByFiveTwoToThePower = (input) => {
  // let newArr = input.map(val => val.filter(val => val%5 === 0 || !isNaN(val)))
  // console.log(newArr.map(val => val ** 2))

  let newArr = input.map(val => val.filter(val => val%5 === 0 && typeof val === 'number'))
  // console.log(newArr.map(val => Math.pow(2, val)))
  let newSquaredArr = newArr.map(val => val.map(num => (2 ** num)))
  // console.log(newSquaredArr)
  return newSquaredArr
};
...
let findMaleAndFemale = (data) => {
  let str = []
  data.forEach(obj => obj.gender === 'male' || obj.gender === 'female' ? str.push(obj.name) : str)
  return str.join(' and ')
};
...
let findShortest = (data) => {
  let characterHeight = data.reduce((acc, cur) => Number(acc.height) > Number(cur.height) ? acc = cur : acc, {height:'3000'})
  console.log(characterHeight)
  return characterHeight.name
};

// challanges done with Yuri and Anthony
