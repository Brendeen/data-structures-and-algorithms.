...
const firstLetters = (arr) => {
  return arr.map(idx => idx.charAt(0));
};
...
const standardizePhoneNumbers = (arr) => {
  let newArr = arr.map(word => word.split('').filter(nan => !isNaN(nan)).join(''))
  return newArr.map(num => num.slice(0, 3) + num.slice(4))
};
